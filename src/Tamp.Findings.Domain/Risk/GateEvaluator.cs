...
public static class GateEvaluator
{
        ...
        RiskInputs? prior, double? priorScore, double? deltaPoints)
    {
        ...
        return key switch
        {
            ...
            GateKeys.HighSast            => Threshold(key, cfg, current.SastHigh, 0, "high SAST"),
            GateKeys.CriticalDast        => Threshold(key, cfg, current.DastCritical, 0, "critical DAST"),
            GateKeys.HighDast            => Threshold(key, cfg, current.DastHigh, 0, "high DAST"),
