...
/// <summary>
/// The CISA Secure Software Development Attestation, built from ingested
/// evidence (TFND-100 … TFND-103).
///
/// Structured to the NIST SSDF (SP 800-218) practice taxonomy that M-22-18 /
/// EO 14028 reference, in four families:
...
///
/// For every practice that maps to data we ingest, this emits Yes/Partial/No
/// plus the concrete evidence. Organizational-only practices are emitted as
/// MANUAL — not as failures. The signing officer answers those from artefacts
/// outside this tool, and a tool that scored them would be inventing an answer.
///
/// Lives in Application, not Api, because the Blazor attestation screen and the
/// export both need it and Web must not reference Api (ADR 0002). The endpoint
/// is now a thin wrapper over this.
/// </summary>
public sealed class SsdfAttestationBuilder
{
    ...
    /// <summary>
    /// Build the document for one project.
    /// </summary>
    /// <param name="commitSha">
    /// The build to attest. Null means the latest canonical build. An
    /// attestation is ABOUT a specific build — "the latest" is a convenience
    /// for the screen, never what gets signed, which is why the returned
    /// document always names the commit it actually used.
    /// </param>
    /// <returns>null when no such project exists.</returns>
    public async Task<SsdfAttestationDoc?> BuildAsync(
        Guid projectId, string? commitSha = null, CancellationToken ct = default)
    {
        ...
        if (canonical.Count == 0) return Empty(project);

        // One "build" is every component version sharing a commit.
        var wanted = commitSha is { Length: > 0 }
            ? canonical.FirstOrDefault(c =>
                  (c.CommitSha ?? c.VersionString).StartsWith(commitSha, StringComparison.OrdinalIgnoreCase))
            : canonical[0];

        // A sha that matches nothing is a broken link, not the latest build.
        // Silently falling back would attest a different commit than the URL
        // names — the single worst thing this document could do.
        if (wanted is null) return Empty(project);

        var topCommit = wanted.CommitSha ?? wanted.VersionString;
        var build = canonical
        ...
        var vexCounts = await _db.VexStatements.AsNoTracking()
            .Where(v => v.ProjectId == projectId && v.RetiredAt == null)
            .GroupBy(v => v.Status)
            .Select(g => new { Status = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.Status, x => x.Count, ct);

        var poamCounts = await _db.PoamItems.AsNoTracking()
        ...
        var latestSbomTools = await _db.SbomSnapshots.AsNoTracking()
        ...
        var receipts = await _db.ScanRunReceipts.AsNoTracking()
            .Where(r => build.Contains(r.ComponentVersionId))
            .Select(r => new { r.Scanner, r.Status })
            .ToListAsync(ct);
        var succeeded = receipts
            .Where(r => r.Status == ScanRunStatus.Succeeded)
            .Select(r => r.Scanner)
            .ToHashSet();
        ...
        var doc = new SsdfAttestationDoc
        {
            ...
            Gates = new(
                ...
                // and it also dropped every Unknown.
    ...
    private static SsdfAttestationDoc Empty(Project project) => new()
    {
        Generated = DateTimeOffset.UtcNow,
        Project = new(project.Id, project.Name, project.Client?.Name ?? ""),
        Build = null,
...
// -------- The document ---------------------------------------------------
//
// Shape is a wire contract: the Api endpoint serialises these verbatim and
// the FedRAMP package consumes the JSON. Renaming a property here changes
// what an assessor's tooling sees.
...
public sealed record VdpEvidence(string? PolicyUrl, string? ContactEmail, string? ReportingFormUrl);
